...
Complete tool catalog for the VGC MCP Server with 208 tools for competitive Pokemon team building.

> The authoritative, always-current tool list is generated by
> `scripts/build_catalog.py` into [tools-catalog.md](tools-catalog.md). This
> page is a curated hand-written reference and may lag; when the two disagree,
> the generated catalog wins.
